...
flags.DEFINE_enum('dataset', 'continuous_mnist',
...
flags.DEFINE_enum('grad_type', 'reinforce',
                  ['disarm', 'reinforce_loo', 'double_cv', 'relax', 'arms', 'Cvor', 'Cvor_AO', 'CVor_Double',
                   # without 'output':
                   #    'mu': H(f(\mu), \nabla f(\mu) * (x - \mu))
                   ...
                   #    'jointavg': avg. of H( f, \nabla f(x_j) *
                   #    (x - x_j), avg. of \nabla f(x_j) * (x - \mu))
                   'discrete_stein_MPF_oldavg', 'discrete_stein_MPF_jointavg', 'discrete_stein_MPF_mu',
                   'discrete_stein_MPF_avg',
                   'discrete_stein_DIFF_oldavg', 'discrete_stein_DIFF_jointavg', 'discrete_stein_DIFF_mu',
                   'discrete_stein_DIFF_avg',
...
flags.DEFINE_integer('num_train_samples', 10,
                     'Number of samples for evaluation.')
flags.DEFINE_bool('debug', False, 'Turn on debugging mode.')
FLAGS = flags.FLAGS


def min_max_normalization(F_value):
    F_min = tf.reduce_min(F_value)
    F_max = tf.reduce_max(F_value)
...
def bootstrap_mean_tf(data, n_bootstrap_samples=1000):
    # Get the number of samples in the data
    num_samples = tf.shape(data)[0]

    # Randomly sample with replacement to generate bootstrap samples
    bootstrap_indices = tf.random.uniform([n_bootstrap_samples, num_samples], minval=0, maxval=num_samples,
                                          dtype=tf.int32)
    bootstrap_samples = tf.gather(data, bootstrap_indices)

    # Calculate the mean of each bootstrap sample
    bootstrap_means = tf.reduce_mean(bootstrap_samples, axis=1)
...
def estimate_gradients(input_batch, bvae_model, gradient_type, hyper_optimizer=None, control_optimizer=None):
    ...
    if gradient_type == 'relax':
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            fb_sum = tf.reduce_sum(f_b, axis=0)

            if gradient_type == "CVor_Double":
                # grad_b: [K, bs, D]
                grad_b = tape.gradient(fb_sum, b) * FLAGS.K
                avg_grad_b = tf.reduce_mean(grad_b, axis=0)
                loo_avg_grad_b = (tf.reduce_sum(grad_b, axis=0) - grad_b) / (FLAGS.K - 1)

                loo_vr = f_b + tf.reduce_sum(loo_avg_grad_b, axis=-1)
                loo_vr = (tf.reduce_sum(loo_vr, axis=0) - loo_vr) / (FLAGS.K - 1)
                # cv: [K, bs]
                cv = tf.reduce_sum(loo_avg_grad_b, axis=-1)
                F_value = (f_b - loo_vr + cv)[..., None]
                F_value = tf.squeeze(F_value, axis=-1) - tf.reduce_sum(avg_grad_b * theta * (1. - theta), axis=-1)
                F_value_detach = tf.identity(tf.stop_gradient(F_value))
                fb_detach = tf.identity(tf.reduce_sum(f_b, axis=0) - f_b)
                bootstrap_mean = tf.identity(bootstrap_mean_tf(F_value_detach))
                # print(F_value)
                # F_value = tf.squeeze(F_value, axis=-1)
                tilde_F_value = tf.reduce_mean(
                    tf.exp(fb_detach * (F_value - F_value_detach) / (tf.reduce_mean(bootstrap_mean)))
                )

                tilde_F_value_detach = tf.stop_gradient(tilde_F_value)
                CVor = tf.exp(
                    tf.exp(tilde_F_value - tilde_F_value_detach) - tf.exp(F_value - F_value_detach)
                )
                # print(CVor)
                CVor_loss = CVor # * f_b

            if gradient_type == "Cvor_AO" and FLAGS.dataset == "continuous_fashion":
                F_value1 = (tf.reverse(f_b, axis=[0]) - (tf.reduce_mean(f_b, axis=0) - f_b) / 1000) / 3
                F_value2 = (tf.reverse(f_b, axis=[0])) / 3
                # F_value = tf.reverse(f_b, axis=[0])
                F_value_detach = tf.stop_gradient(F_value1)
                fb_detach = tf.identity(F_value2)
                bootstrap_mean = tf.identity(bootstrap_mean_tf(F_value_detach))
                tilde_F_value = tf.reduce_mean(
                    tf.exp(fb_detach * (F_value1 - F_value_detach) / (tf.reduce_mean(bootstrap_mean)))
                )
                tilde_F_value_detach = tf.stop_gradient(tilde_F_value)
                CVor = tf.exp(
                    tf.exp(tilde_F_value - tilde_F_value_detach) - tf.exp(F_value1 - F_value_detach)
                )

                CVor_loss = CVor * f_b

            elif gradient_type == "Cvor_AO":
                F_value = tf.reverse(f_b, axis=[0])
                F_value_detach = tf.identity(tf.stop_gradient(F_value))
                fb_detach = tf.identity(tf.reduce_sum(f_b, axis=0) - f_b)
                bootstrap_mean = tf.identity(bootstrap_mean_tf(F_value_detach))
            ...
            if gradient_type == "Cvor":
                # Expand the dimensions of theta to match the dimensions of b
                theta_expanded = tf.expand_dims(theta, 0)  # Expands theta.shape to (1, 100, 200)
                theta_expanded = tf.broadcast_to(theta_expanded,
                                                 b.shape)  # Expands theta_expanded.shape to (2, 100, 200)

                # Calculate theta raised to the power of b, and (1 - theta) raised to the power of (1 - b)
                pow_theta = tf.reduce_mean(tf.pow(theta_expanded, b), axis=2)
                pow_one_minus_theta = tf.reduce_mean(tf.pow(1 - theta_expanded, 1 - b),
                                                     axis=2)  # Ensures theta_expanded's shape is the same as b

                # Multiply corresponding elements
                f_b = f_b * pow_theta
                f_b = f_b * pow_one_minus_theta
                f_b_shape = f_b.get_shape().as_list()

                # Define weights and biases
                weights = tf.random.normal([f_b_shape[1], f_b_shape[1]])  # Shape of weight matrix is [100, 100]
                biases = tf.random.normal([f_b_shape[1]])  # Shape of bias vector is [100]

                # Perform linear layer computation
                F_value = tf.matmul(f_b, weights) + biases
                F_value = min_max_normalization(F_value) * 40
                F_value_detach = tf.stop_gradient(F_value)
                fb_detach = tf.stop_gradient(F_value)
                bootstrap_mean = bootstrap_mean_tf(F_value_detach)
                tilde_F_value = tf.reduce_mean(tf.exp(fb_detach * (F_value - F_value_detach) / (tf.reduce_mean(bootstrap_mean) )))
                tilde_F_value_detach = tf.stop_gradient(tilde_F_value)
                CVor = tf.exp(tilde_F_value - tilde_F_value_detach - F_value + F_value_detach)
        ...
        if gradient_type == "double_cv":
            ...
            infnet_grads = tape.gradient(eta, bvae_model.encoder_vars, output_gradients=eta_grads)
            ...
            with tf.GradientTape() as hyper_tape:
                eta_grads = bvae_model.get_grad_estimation(
                    input_batch, f_b, b, eta, theta, u, grad_type=gradient_type,
                    grad_b=grad_b)  # , extra_params=FLAGS.discrete_stein)
                variance_loss = tf.reduce_mean(tf.square(eta_grads))
            ...
            if gradient_type in ["Cvor", "Cvor_AO", "CVor_Double"]:
                # If the gradient type is one of the specified types, set eta_grads to None
                # and compute gradients with respect to encoder variables using CVor_loss
                eta_grads = None
                infnet_grads = tape.gradient(CVor_loss, bvae_model.encoder_vars)
            else:
                # For other gradient types, compute eta_grads using the get_grad_estimation method
                # of the bvae_model and then compute infnet_grads using these eta_grads
                eta_grads = bvae_model.get_grad_estimation(
...
def main(grad_type, test_run=False):
    ...
    def train_one_step(
        ...
        if grad_type == 'relax':
            ...
            (genmo_grads, prior_grads, infnet_grads, eta_grads, genmo_loss) = estimate_gradients(
                input_batch, bvae_model, grad_type, hyper_optimizer=hyper_optimizer,
            ...
            infnet_vars = bvae_model.encoder_vars
            infnet_optimizer.apply_gradients(list(zip(infnet_grads, infnet_vars)))
            if grad_type == "Cvor" or grad_type == "Cvor_AO" or grad_type == "CVor_Double":
                eta_grad_var = None
            else:
    ...
    if test_run:
        step_freq = 200
        steps = tqdm(range(start_step, 200*10000))
    else:
...
def test_run(_):
    ...
    losses = {}
    alphas = {}
    # grad_types = ["discrete_stein_avg", "discrete_stein_mu", "double_cv", "reinforce_loo"，"reinforce", "Cvor_AO", "Cvor"]
    grad_types = ["reinforce"]
    ...
    for grad_type in grad_types:
        ...
        df_variances[grad_type] = variances[grad_type]
    ...
    plt.legend()
    plt.title("Variance")
    plt.savefig("var_test.pdf", bbox_inches="tight")

    plt.figure()
    for grad_type in grad_types:
        print(losses[grad_type][:100])

        df_losses[grad_type] = losses[grad_type]
    ...
    plt.legend()
    plt.title("Loss")
    plt.savefig("loss_test.pdf", bbox_inches="tight")

    df_variances.to_excel("variances_test.xlsx")
    df_losses.to_excel("losses_test.xlsx")
