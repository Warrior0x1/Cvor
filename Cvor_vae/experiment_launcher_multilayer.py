...
# import dataset_multilayer as dataset
import dataset
import networks_multilayer as networks
from networks import SteinControlNNSingleLayer
...
flags.DEFINE_enum('grad_type', 'disarm',
                  ['arm', 'disarm', 'reinforce_loo', 'relax', 'double_cv',
...
flags.DEFINE_string('logdir', os.getenv('PT_OUTPUT_DIR', "./logs/multi_" + datetime.now().strftime("%Y%m%d%H%M%S")),
...
def estimate_gradients(input_batch, bvae_model, gradient_type, hyper_optimizer=None, control_optimizer=None):
  ...
  if gradient_type == 'relax':
    ...
    def get_infnet_grad(gradient_type):
      ...
      return infnet_grad_multiplier

    if gradient_type == 'double_cv':
      with tf.GradientTape() as hyper_tape:
        infnet_grad_multiplier = get_infnet_grad(gradient_type)
        variance_loss = tf.reduce_mean(tf.square(infnet_grad_multiplier))
...
def train_one_step(
  ...
  if FLAGS.grad_type == 'relax':
    ...
    (genmo_grads, prior_grads, infnet_grads, genmo_loss) = estimate_gradients(
        input_batch, bvae_model, FLAGS.grad_type, hyper_optimizer=hyper_optimizer, control_optimizer=control_optimizer)
...
def main(_):
  ...
  if grad_type == 'relax':
    ...
    control_network = []
    for _ in range(len(encoder)):
      control_network_i = SteinControlNNSingleLayer()
      control_network.append(control_network_i)
  else:
  ...
  tensorboard_file_writer = tf.summary.create_file_writer(FLAGS.logdir)
  ...
  for step_i in range(start_step, FLAGS.num_steps):
    ...
    if step_i % 1000 == 0:
      timer += time.time()
      tf.print("time:", timer / 1000 * 1000, 'ms/iter')
      metrics.update({
      ...
      if FLAGS.grad_type == 'relax':
        ...
        for i in range(bvae_model.num_layers):
          metrics['double_cv/alpha_' + str(i)] = bvae_model.alpha[i]
      elif grad_type.startswith("discrete_stein"):
        for i in range(bvae_model.num_layers):
          metrics['double_cv/alpha_' + str(i)] = bvae_model.alpha[i]
          metrics['double_cv/beta_' + str(i)] = bvae_model.beta[i]
