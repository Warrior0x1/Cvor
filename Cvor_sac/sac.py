...
class SAC(object):
    ...
    def bootstrap_mean_cuda(self, data, n_bootstrap_samples=1000):
        # 获取数据的样本数
        num_samples = data.shape[0]

        # 随机抽样（有放回）来生成 bootstrap 样本
        bootstrap_indices = torch.randint(0, num_samples, (n_bootstrap_samples, num_samples), device=data.device)
        bootstrap_samples = data[bootstrap_indices]

        # 计算每个 bootstrap 样本的均值
        bootstrap_means = torch.nanmean(bootstrap_samples, dim=0)

        # 计算所有 bootstrap 样本均值的平均值
